		...
		if (total) {
			...
			for (int i = 0; i < read; ++i , ++cur) {
				...
				if (cur->usri2_last_logon) 
					printf("  login: %s",ctime((const long*)&cur->usri2_last_logon));
	
				if (cur->usri2_last_logoff) 
					printf("  logoff: %s",ctime((const long*)&cur->usri2_last_logoff));

				if (cur->usri2_acct_expires != TIMEQ_FOREVER)
					printf("  expires: %s\n", ctime((const long*)&cur->usri2_acct_expires));
...
void enum_groups(char *s, int d) {
	...
	do {
		ret = NetLocalGroupEnum(wserver, 0, (unsigned char **)&buff, 1024, &read, &total, &resume);
...
void lsastrcpy(PLSA_UNICODE_STRING lsastr, LPWSTR str) {
	if(!str) {
		...
		lsastr->Buffer = str;
		lsastr->Length = (USHORT)lstrlenW(str) * sizeof(WCHAR);
